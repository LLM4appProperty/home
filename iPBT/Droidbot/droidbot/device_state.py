...
class DeviceState(object):
    ...
    def save_all_views_img(self, output_dir=None,app=None):
        """
        Save all views' images in the current state
        :param output_dir: str, the directory to save the images
        """
        try:
            if output_dir is None:
                if self.device.output_dir is None:
                    ...
                    output_dir = os.path.join(self.device.output_dir, "allviews")
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            for view_dict in self.views:
                # if view_dict['children'] != []:
                #    continue  # skip views with children
                if view_dict['package'] != app.package_name:
                    continue  # skip views from other packages
                if view_dict['resource_id'] is None and view_dict['text'] is None and view_dict['content_description'] is None:
                    continue  # skip views without resource_id, text or content_description
                view_str = view_dict['view_str']
                if self.device.adapters[self.device.minicap]:
                    view_file_path = "%s/view_%s.jpg" % (output_dir, view_str)
                else:
                ...
                # Load the original image:
                view_bound = view_dict['bounds']
                # ignore the view if its bounds are a line or a point or less than 0
                #if view_bound[0][0] == view_bound[1][0] or view_bound[0][1] == view_bound[1][1] or view_bound[0][0] < 0 or view_bound[0][1] < 0 or view_bound[1][0] < 0 or view_bound[1][1] < 0:
                #    self.device.logger.warning("View %s has invalid bounds %s, skip saving." %
                #                               (view_str, view_bound))
                #    continue
                original_img = Image.open(self.screenshot_path)
                # view bound should be in original image bound
